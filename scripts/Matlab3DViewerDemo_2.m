...
% which location and size is set randomly. From this volume, we ask the 3D
...
for i = 1 : nballs
    ...
    %%
    % Logical indexing: we find the pixel indices that are within object
    % volume.
    ...
    % |javax.vecmath package|. We determine its actual color randmoly
    % again.
    ...
    % assumes that there is threshold that separates the background from
    ...
    % would have worked. 
    %
    % The 3 boolean array is used to specify what channels will be included
    % in the surface rendering, if you feed it a color image. In our case,
    % we simply set them all to true and do not care much. Finally, the
    % resampling factor is used to win some time and memory at the expense
    % of precision. If set to more than 1, the image is downsampled before
    % the marching cube algorithm is applied, resulting in less meshes
    % obtained in smaller time.
