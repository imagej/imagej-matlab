...
% http://pacific.mpi-cbg.de/wiki/index.php/Main_Page) is a _distribution_
% of ImageJ, that include(amongst other things) a lot of plugins, some
...
% etc. It is also shipped with *Miji*, a set of utilities made to ease its
...
% Virtual Machine. Fortunately, Fiji is shipped with a script that
...
%% Installing Fiji
%
% Simply go to http://pacific.mpi-cbg.de/wiki/index.php/Downloads and
% download the binaries for your OS. You will not find an installer; you
% just have to unzip the Fiji folder to somewhere convenient for you.
%
...
% in scientific Image Processing. There is a zillion things you can do with
% it. If you are interested in using Fiji as a standalone application,
% check some of the tutorials there:
% http://pacific.mpi-cbg.de/wiki/index.php/Documentation.
%
...
%
% If you get a message stating that the function cannot be done, this is
% because you did not add the Fiji |scripts| folder to the MATLAB path. in
% that case, go back to the previous paragraph.
% 
% The |InstallJava3D| will tell you what it does verbosely. It will check
% whether your JVM already has Java3D installed, and if the version
% requirement is met. If not, it will automatically download it and install
...
% writable. Then you can simple re-launch the script.
%
...
% the same folder than this file.
%
% * |Matlab3DViewerDemo_1| shows how to load a 3D data in MATLAB and render
% it in 3D (volumetric rendering);
% * |Matlab3DViewerDemo_2| shows how to create simple objects volumes in
...
% * The ImageJ 3D viewer is written by Benjamin Schmidt and Johannes
% Schindelin;
% * MIJ is written by Daniel Sage;
